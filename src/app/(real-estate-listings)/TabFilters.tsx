...
const TabFilters = () => {
  ...
  useEffect(() => {
    ...
    if (distance) {
      ...
      // Convert to the range used by the price slider (for display purposes)
      const minValue = parseFloat(min) || 0;
      const maxValue = parseFloat(max) || 1000;
      setRangePrices([minValue * 20, maxValue * 20]); // Scale for UI display
    }
    
    // Process other filters as needed
    const category = searchParams?.get("category");
    if (category) {
      // Set other filters based on category
    }
  }, [searchParams]);
  ...
  const renderTabsPriceRage = () => {
    // Use the distance range from search params if available
    const distance = searchParams?.get("distance");
    let displayText = `$${convertNumbThousand(rangePrices[0])} - $${convertNumbThousand(rangePrices[1])}`;
    
    if (distance) {
      const [min, max] = distance.split("-");
      displayText = `${min} - ${max} km`;
    }
